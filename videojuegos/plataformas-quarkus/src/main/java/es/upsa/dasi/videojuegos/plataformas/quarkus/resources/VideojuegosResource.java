package es.upsa.dasi.videojuegos.plataformas.quarkus.resources;

import es.upsa.dasi.videojuegos.exceptions.VideojuegoException;
import es.upsa.dasi.videojuegos.plataformas.quarkus.services.Service;

import javax.inject.Inject;
import javax.ws.rs.GET;
import javax.ws.rs.Path;
import javax.ws.rs.PathParam;
import javax.ws.rs.Produces;
import javax.ws.rs.core.MediaType;
import javax.ws.rs.core.Response;

@Path("games")
public class VideojuegosResource {

    @Inject
    Service service;

    @GET
    @Path("{id}/plataformas")
    @Produces(MediaType.APPLICATION_JSON)
    public Response getPlataformasByVideojuego(@PathParam("id") String idVideojuego) throws VideojuegoException
    {
        return Response.ok()
                .entity(  service.getPlataformasVideojuego(idVideojuego) )
                .build();
    }
}
